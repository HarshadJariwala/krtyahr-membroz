...
export default LeaveScreen = (props) => {
    ...
    const onPressToSelectExpandLeave = (item, index, val) => {
        const leave = myLeaveFilterList.map((item) => {
            item.selected = false;
            return item;
        });
        if (val == false) {
            leave[index].selected = false;
        }
        if (val == true) {
            leave[index].selected = true;
        }
        setMyLeaveFilterList(leave);
    }
